...
function App() {
 ...
 const [properties, setProperties] = useState<Property[]>([
  ...
  {
    id: 3,
    name: 'Himalaya Towers',
    address: '78 MG Road, Pune, Maharashtra',
    type: 'apartment',
    units: 18,
    rent: 28000,
    status: 'occupied',
  ...
  },
  {
    id: 5,
    name: 'Sundar Villa',
    address: '34 Residency Road, Hyderabad, Telangana',
    type: 'villa',
    units: 10,
    rent: 27000,
    status: 'maintenance',
  ...
  },
  {
    ...
    address: '78 Brigade Road, Bengaluru, Karnataka',
    type: 'apartment',
    units: 22,
    rent: 32000,
    status: 'occupied',
  ...
  {
    id: 8,
    name: 'Lotus Residency Tower B',
    address: '125 MG Road, Bengaluru, Karnataka',
    type: 'apartment',
    units: 18,
    rent: 25000,
    status: 'maintenance',
...
const [tenants, setTenants] = useState<Tenant[]>([
  ...
  },
  {
    ...
    propertyName: 'Marigold Residency',
    unit: 'F-302',
  ...
  const [leases, setLeases] = useState<Lease[]>([
  {
    ...
    endDate: '2024-12-31',
    monthlyRent: 25000, // INR
    securityDeposit: 50000, // INR
    status: 'active',
    terms: 'Standard residential lease terms apply. No pets allowed. Water and maintenance included. Electricity billed separately.'
  },
  {
    id: 2,
    ...
    status: 'active',
    terms: 'Standard lease. Pets allowed with prior approval. Maintenance included. Electricity and gas separate.'
  },
  {
    id: 3,
    ...
    status: 'active',
    terms: 'Residential lease with 1-year term. No smoking allowed inside units. Maintenance included. Electricity separate.'
  },
  {
    id: 4,
    tenantName: 'Priya Kapoor',
    propertyName: 'Shivneri Apartments',
    unit: 'D-402',
    ...
    status: 'active',
    terms: 'Lease term: 1 year. Pets allowed. Water and maintenance included. Electricity billed separately.'
  }
]);
const [payments, setPayments] = useState<Payment[]>([
  ...
  const handleSignup = (name: string, email: string, password: string) => {
    // In a real app, you would create user account with backend
    setIsAuthenticated(true);
  };
  ...
  const editProperty = (updatedProperty: Property) => {
  ...
  const addLease = (lease: Omit<Lease, 'id'>) => {
    const newLease = { ...lease, id: Date.now() };
    setLeases([...leases, newLease]);
  };

  const editLease = (updatedLease: Lease) => {
    ...
    <div className="app min-h-screen bg-gray-50">
      ...
      <div className="main-content">
        ...
        {currentView === 'tenants' && (
          <Tenants
            tenants={tenants}
            properties={properties.map(p => ({ id: p.id, name: p.name, units: p.units }))}
        ...
        {currentView === 'leases' && (
          <Leases
            leases={leases}
            tenants={tenants.map(t => ({ id: t.id, name: t.name, propertyName: t.propertyName, unit: t.unit }))}
            onAddLease={addLease}
        ...
        {currentView === 'payments' && (
          <Payments
            payments={payments}
            tenants={tenants.map(t => ({ id: t.id, name: t.name, propertyName: t.propertyName, unit: t.unit, rent: t.rent }))}
